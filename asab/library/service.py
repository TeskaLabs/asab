...
class LibraryService(Service):
	...
	* `azure+https://` for Microsoft Azure Storage provider.
	...
	async def read(self, path: str, tenant: str = None) -> typing.IO:
		...
		"""
		# It must start with '/'
		assert path[:1] == '/', "File path must start with '/'. Example: /library/sample.html"
	...
	async def list(self, path="/", tenant=None, recursive=False):
		...
		assert path[:1] == '/', "Directory path must start with '/'. Example: /library/Templates/"
		# Path must end with '/'
		assert (path[-1:]) == '/', "Directory path must end with '/'. Example: /library/Templates/"
